// sheets.js (ESM) — drop-in fix para "Identifier 'SPREADSHEET_ID' has already been declared"
...
export const SPREADSHEET_ID =
  process.env.SHEETS_SPREADSHEET_ID || process.env.SHEETS_ID || '';

if (!SPREADSHEET_ID) {
  throw new Error('Falta SHEETS_SPREADSHEET_ID (o SHEETS_ID) en el entorno.');
}
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (tel === phone) {
      ...
      return {
        ...
        dep,
        subzona: zona,
        updatedTs: toTs(updatedStr),
        campanaUpdatedTs: toTs(campUpdStr),
      };
    }
  }
...
function buildRowFromSession(s, fromPhone, estado = 'NUEVO') {
  ...
  const estadoFinal =
    EST === 'NUEVO' || EST === 'PENDIENTE' || EST === 'CERRADO' ? EST : 'NUEVO';
  const seguimiento = '';
  const calId = '';
...
export function campanaFromNow() {
  const m = monthNowTZ();
...
function dmyFromOffset(days) {
  ...
  return `${pad2(tgt.getUTCDate())}/${pad2(tgt.getUTCMonth() + 1)}/${pad2(
    tgt.getUTCFullYear()
  )}`;
}
...
function nextDateFromWords(text) {
  ...
  const m = t.match(
    /\b(este|prox(?:imo)?)?\s*(domingo|lunes|martes|miercoles|jueves|viernes|sabado)\b/
  );
  if (m) {
    const qualifier = m[1] || '';
    const dayName = m[2];
...
function dateFromDayMonthWords(text) {
  ...
  const m = t.match(
    /\b([0-3]?\d)\s*(?:de\s*)?([a-záéíóúñ]{3,12})\.?(?:\s*de\s*(\d{2,4}))?\b/
  );
  if (!m) return '';
...
export function parseClientResponse(text = '', fallbackName = '') {
  ...
  const rePlaca =
    /(placa(?:\s+del\s+veh[ií]culo)?|placa)\s*[:\-]\s*([A-Za-z0-9\-\s]{4,})/i;
  const reFecha =
    /(fecha(?:\s+de)?\s*(recojo|retiro)?)(?:\s*\([^)]*\))?\s*[:\-]\s*([0-3]?\d[\/\-][01]?\d[\/\-]\d{2,4})/i;

  const reCI =
    /(c\.?\s*i\.?|ci|carnet(?:\s+de)?\s+identidad|cedula|c[eé]dula)(?:\s+(?:del|de)\s+chofer)?\s*[:\-]\s*([A-Za-z0-9.\-\/\s]+)/i;
  ...
  for (const line of lines) {
    ...
    if (!out.ciChofer) {
      const m = line.match(reCI);
      if (m) out.ciChofer = onlyDigits(m[2]);
    }
    if (!out.placa) {
  ...
  if (!out.ciChofer) {
    const mCI = String(text).match(
      /(?:c\.?\s*i\.?|ci|carnet(?:\s+de)?\s+identidad|cedula|c[eé]dula)[^0-9]{0,15}([0-9.\-\/\s]{5,})/i
    );
  ...
  const labeledHints =
    /(raz[oó]n|rs|nit|chofer|conductor|placa|fecha|cliente|carnet|ci|cedula|c[eé]dula)\s*[:\-]/i;
  ...
  if (!out.razonSocial) {
    const m =
      text.match(/rs\s*[:\-]\s*([^\n;]+)/i) ||
      text.match(/raz[oó]n\s*social\s*[:\-]\s*([^\n;]+)/i);
...
export async function appendBillingPickupRow({
  nombreCliente,
  razonSocial,
  nit,
  nombreChofer,
...
/* ==========================
   HOJA 3 (PRECIOS) & HOJA 4 (HIST)
   ========================== */
const TAB3_PRECIOS = process.env.SHEETS_TAB3_NAME || 'Hoja 3';
const TAB4_HIST = process.env.SHEETS_TAB4_NAME || 'Hoja 4';

const PRECIOS_VERSION_CELL =
  process.env.SHEETS_PRICES_VERSION_CELL || `${TAB3_PRECIOS}!J1`;
const PRECIOS_RATE_CELL =
  process.env.SHEETS_PRICES_RATE_CELL || `${TAB3_PRECIOS}!J2`;

const TAB3_PRECIOS_PUBLIC = process.env.SHEETS_TAB3_NAME || 'Hoja 3';
const TAB3_PRECIOS_PRIVATE =
  process.env.SHEETS_TAB3_PRIVATE_NAME || 'PRECIOS_NUEVOS';
...
// Export utilitaria por si la necesitas externamente
export { buildRowFromSession };
